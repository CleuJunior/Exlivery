defmodule Exlivery.Orders.CreateOrUpdate do
  ...
  def call(%{user_cpf: user_cpf, items: items}) do
    with {:ok, user} <- UserAgent.get(user_cpf), {:ok, items} <- build_items(items), {:ok, order} <- Order.build(user, items) do
  ...
  defp build_items(items), do: items |> Enum.map(&build_item/1) |> handle_build()
  ...
  defp handle_build(items), do: if Enum.all?(items, &is_struct/1), do: {:ok, items}, else: {:error, "Invalid Items"}
